...
async function load(){
 ...
 $("deals").innerHTML=(d.results||[]).slice(0,15).map(x=>`<div class="deal"><div><b>${x.origin} → ${x.destination}</b><div class="muted">${x.departure} a ${x.return}</div></div><div class="price">${money(x.price,x.currency)}</div></div>`).join("")||'<div class="muted">Nenhuma oferta.</div>';
